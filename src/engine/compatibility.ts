...
/**
 * Règle portant sur une valeur estimée (base étendue) : l'erreur devient un avertissement « à vérifier ».
 * Chaque paire [composant, champ] indique la donnée utilisée par la règle.
 */
const soft =
  (...refs: [{ estimated?: string[] } | undefined, string][]) =>
  (message: string, ...categories: ComponentCategory[]): CompatibilityIssue => {
    const guessed = refs.some(([c, field]) => c?.estimated?.includes(field))
    return guessed
      ? { severity: 'warning', message: `${message} (valeur estimée, à vérifier)`, categories }
      : { severity: 'error', message, categories }
  }
...
export function checkResolved(b: ResolvedBuild, deviceType: DeviceType = 'desktop'): CompatibilityIssue[] {
  ...
  if (pcCase) {
    for (const g of gpus)
      if (g.lengthMm > pcCase.maxGpuLengthMm) {
        out.push(soft([g, 'lengthMm'], [pcCase, 'maxGpuLengthMm'])(`${g.chipset} trop longue (${g.lengthMm} mm > ${pcCase.maxGpuLengthMm} mm max).`, 'gpu', 'case'))
        break
      }
  }
  ...
  if (mb && gpus.length > mb.pcieX16Slots)
    out.push(soft([mb, 'pcieX16Slots'])(`${gpus.length} cartes graphiques pour ${mb.pcieX16Slots} port(s) PCIe x16.`, 'gpu', 'motherboard'))
  ...
  if (cooler && cpu) {
    if (!cooler.sockets.includes(cpu.socket))
      out.push(soft([cooler, 'sockets'])(`Le ventirad ne supporte pas le socket ${cpu.socket}.`, 'cooler', 'cpu'))
  ...
  if (cooler && pcCase) {
    if (cooler.type === 'air' && cooler.heightMm && cooler.heightMm > pcCase.maxCoolerHeightMm)
      out.push(soft([cooler, 'heightMm'], [pcCase, 'maxCoolerHeightMm'])(`Ventirad trop haut (${cooler.heightMm} mm > ${pcCase.maxCoolerHeightMm} mm).`, 'cooler', 'case'))
    if (cooler.type === 'aio' && cooler.radiatorMm && cooler.radiatorMm > pcCase.maxRadiatorMm)
      out.push(soft([pcCase, 'maxRadiatorMm'])(`Radiateur ${cooler.radiatorMm} mm non supporté (max ${pcCase.maxRadiatorMm} mm).`, 'cooler', 'case'))
  }
  ...
  if (psu && pcCase) {
    if (pcCase.psuFormFactor === 'Redundant' && psu.formFactor !== 'Redundant')
      ...
      out.push(soft([pcCase, 'psuFormFactor'])(`Le boîtier n’accepte qu’une alimentation SFX.`, 'psu', 'case'))
    else if (pcCase.psuFormFactor === 'SFX-L' && !['SFX', 'SFX-L'].includes(psu.formFactor))
      out.push(soft([pcCase, 'psuFormFactor'])(`Le boîtier n’accepte qu’une alimentation SFX / SFX-L.`, 'psu', 'case'))
  ...
  if (mb && sata + sas > sataPorts)
    out.push(soft([mb, 'sataPorts'])(`${sata + sas} disques SATA/SAS pour ${sataPorts} port(s) disponibles : ajoutez un HBA.`, 'storage', 'hba'))
...
export function withComponent(slots: BuildSlots, c: PCComponent, mode: 'replace' | 'add' = 'replace'): BuildSlots {
  ...
  if (c.category === 'accessory') {
    next.accessories = [...(slots.accessories ?? []), c.id]
  } else if (c.category === 'gpu' || c.category === 'storage') {
...
export function autoComplete(slots: BuildSlots, catalog: Catalog, deviceType: 'desktop' | 'server' | 'nas'): BuildSlots {
  ...
  for (const cat of order) {
    const current = next[cat as Exclude<ComponentCategory, 'accessory'>]
